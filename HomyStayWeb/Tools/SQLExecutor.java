...
public class SQLExecutor {
    ...
    private boolean isUseConnectionManger=false;
    ...
    public <T> boolean ResetIdForPrimaryKey(String tableName, Class<T> target) {
        ...
        try {
            ...
            if (!list.isEmpty()) {
                ...
                SQLPrepared[] updatePrepared=new SQLPrepared[list.size()];
                // 遍历每条记录并更新ID
                for (T t : list) {
                    ...
                    // 仅当ID需要变更时才更新
                        updateSql[newId-1] = "UPDATE " + tableName + " SET id = ? WHERE id = ?";
                        updatePrepared[newId-1] = new SQLPrepared(updateSql[newId-1], newId, oldId);
                    newId++;
                }
                return executeTransaction(updatePrepared);
            }
    ...
    private void ensureConnection() throws SQLException {
        if (connection == null||connection.isClosed()) {
            ...
            isUseConnectionManger=true;
        }
    }
    private void destroyConnection() throws SQLException {
        if(connection==null) return;
        else   if(!isUseConnectionManger) connection.close();
        else connectionManager.releaseConnection(connection);
    }
    ...
    public Object[] getParams(Object...params){ return params;}
}
/*
SQLExecutor
 ├── 核心成员
